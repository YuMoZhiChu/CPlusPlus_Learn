...
public:
	int maxProfit(vector<int>& prices) {
		...
		// 因为存在 1 天的冷冻期, 我们的持有递推关系变为
		// 上一天 keep, 或者今天 卖了, 卖股票不变
		// dp[i][k][0] = max{dp[i-1][k][0], dp[i-1][k-1][1] + prices[i]}
		// 假设 i 天, 持有股票
		// 上一天 keep, 或者今天 买了(那么只能是 前天是不持股的状态
		// (这个不持股的状态 dp[i-2][k-1][0] 可以递推到 dp[i-1][k][0]))
		...
		int dp_pre_0 = 0; // dp[i-2][0] 的存储
		for (int i = 1; i < prices.size(); ++i)
		{
			int temp = dp_i_0; // 代替 dp_pre_0
